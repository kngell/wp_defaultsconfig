...
class FormsController extends Controller
{
    ...
    public function showAll()
    {
        if ($this->request->exists('post')) {
            ...
            $output = $tableClass::$tableHTML(H::getShowAllData($this->model_instance[$table], $this->request, $_REQUEST));
    ...
    public function showDetails()
    {
        if ($this->request->exists('post')) {
            ...
            $this->get_model(str_replace(' ', '', ucwords(str_replace('_', ' ', $table))) . 'Manager', $table);
            $output = H::getShowAllData($this->model_instance[$table], $this->request, $_REQUEST);
            if ($output) {
                $this->jsonResponse(['result' => 'success', 'msg' => $output]);
            } else {
        ...
        if ($this->request->exists('post')) {
            ...
            if ($file['success']) {
                ...
                if ($this->model_instance[$table]->validationPasses()) {
                    ...
                    $errors = $this->renameKeys($this->model_instance[$table]->getErrorMessages(), H::get_Newkeys($this->model_instance[$table], $this->request->getAll('frm_name')));
