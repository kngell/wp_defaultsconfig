...
Gobbledegook check.  You sometimes get spam consisting of gobbledegook characters (� � � etc).  This check will prevent any messages getting through if gobbledegook is found.  This check is done by default.  To disable it, change the value to 0, like so: $gobbledegook_check = 0;
...
// Give an error if $_REQUEST['identiPIC_selected'] is not set, a bot might not send it.  If set then give an error if it is not identical to the $identiPIC array.
    ...
    if (!isset($_REQUEST['identiPIC_selected'])) {
        $errors[] = "You have failed to identify the pictures correctly.  Please try again.";
    } else {
        if ($_REQUEST['identiPIC_selected'] !== $identiPIC) {
            $errors[] = "You have failed to identify the pictures correctly.  Please try again.";
        }
        unset($_REQUEST['identiPIC_selected']);
    }
}
...
if ($required_fields_check) {
    foreach ($required_fields as $value) {
        if ((!isset($_REQUEST[$value]) || empty($_REQUEST[$value])) && (!isset($_FILES[$value]['name']) || empty($_FILES[$value]['name']))) {
    ...
    if ($block_file_types) {
        foreach (array_keys($_FILES) as $value) {
            if (!empty($_FILES[$value]['name'])) {
                foreach ($file_types_to_block as $value2) {
                    if (strtolower(strrchr($_FILES[$value]['name'], ".")) == $value2) {
                        $errors[] = "{$value2} file types are not permitted.  The file \"{$_FILES[$value]['name']}\" cannot be uploaded.";
                    }
                }
            }
        }
...
// Allow posted HTML code to be displayed (and not executed).  Convenient time to do nl2br.
    ...
    {
        if (!is_array($element_value)) {
            $element_value = nl2br(htmlentities($element_value));
        } else {
            foreach ($element_value as $key => $value) {
