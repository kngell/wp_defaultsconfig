...
class Ecommerce {
  ...
  _togglePayment = () => {
    ...
    $(".payment-gateway-header input[type='radio']").each(function() {
      let $this = $(this);
      if (!$this.is(':checked')) {
        $this.closest(".payment-gateway").find(".payment-gateway-content").slideUp(0);
      }
      $this.on("change", function(e) {
        ...
        if (!$this.is(':checked')) {
          $this.closest(".payment-gateway").find(".payment-gateway-content").stop().slideUp(200);
        } else {
          $this.closest(".payment-gateway").find(".payment-gateway-content").stop().slideDown(200);
        }
      })
    });
  }
  ...
  _toggleBillingAddress = () => {
    ...
    $(".billing-address-header input[type='radio']").each(function() {
      let $this = $(this);
      if (!$this.is(':checked')) {
        $this.closest(".billing-address").find(".billing-address-content").slideUp(0);
      }
      $this.on("change", function(e) {
        ...
        if (!$this.is(':checked')) {
          $this.closest(".billing-address").find(".billing-address-content").stop().slideUp(200);
        } else {
          $this.closest(".billing-address").find(".billing-address-content").stop().slideDown(200);
        }
      })
    });
  }
